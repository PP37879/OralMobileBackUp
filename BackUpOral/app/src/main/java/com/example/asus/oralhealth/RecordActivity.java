...
//            myButton[y+24].setBackgroundResource(R.color.white);
    ...
    public void playVoice() {
        if (playlistIndex == 0 || counter == 4) {
            check = false;
        }
        if (check) {
            player = new MediaPlayer();
            if (frontRow == 1 || frontRow == 3) {
                Log.d("FILENAMEB4PLAY : ",fileName);
                Log.d("MESSAGEB4PLAY :",index+"");
                fileName = "sound" + frontRow + playlistIndex;
                Log.d("FILENAMEAFTER : ",fileName);
                Log.d("MESSAGEAFTER :",index+"");
            ...
            } else if (frontRow == 2 || frontRow == 4) {
                String fileName = "sound" + frontRow + playlistIndex;
                int id = getResources().getIdentifier(fileName, "raw", getPackageName());
                player = MediaPlayer.create(this, id);
                player.setLooping(false);
                player.start();
    ...
    public void onResult(Hypothesis hypothesis) {
        if (hypothesis != null) {
            ...
            Log.d("FILENAMERESULT : ",fileName);
            Log.d("MESSAGERESULT :",index+"");
           if(index < 16){
...
//            },1000);
            ...
            playVoice();
            Log.d("FILENAMESEARCHNAME : ",fileName);
            recognizer.startListening(searchName);
            Log.d("MESSAGESEARCHNAME :",index+"");
        }
